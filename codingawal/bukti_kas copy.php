...
if (isset($_POST['no_transaksi']) && isset($_POST['jenis_transaksi'])) {
    ...
    if ($sql) {
        ...
        if (!empty($rows)) {
            ...
            //         $pdf->MultiCell(85, 7, strtoupper($row['kode_akun']) . '. ' . strtoupper($row['nama_akun']) . ' / ' . strtoupper($row['nama_jenjang']) .  ' / ' . strtoupper($firstRow['tahun_ajaran']), 1, 'L'); 
            //         $pdf->SetY($currentY1);
            //         $pdf->SetX(95);
            //         $pdf->MultiCell(75, 7, strtoupper($row['keterangan']), 1, 'L');    
            //         $currentY1 += 14;
            //         $pdf->SetY($currentY2);        
            //         $pdf->SetX(170);
            //         $pdf->SetFont('Arial', '', 9);               
            ...
            foreach ($rows as $row) {
                if (empty($row['tahun_ajaran'])) {
                    ...
                    $tinggi = (strlen($keterangan) <= 30) ? 14 : 7;
                    ...
                    $currentY2 += ($tinggi); // Adjust for the extra height
                }
            }
