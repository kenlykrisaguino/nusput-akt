            ...
            <option value="<?= $kode_akun ?>" data-bank="<?= $akun ?>" data-saldo-normal="<?= $saldo_normal ?>"><?= $kode_akun . ' → ' . $akun ?></option>
              ...
              <th><input style="text-align:center" type="text" class="form-control" name="saldo1" id="saldo1" value="0" oninput="formatNumber(this); calculateTotal();" ></th>
                  ...
                  <option value="<?= $kode_akun ?>" data-bank="<?= $akun ?>" data-saldo-normal="<?= $saldo_normal ?>"><?= $kode_akun . ' → ' . $akun ?></option>
                    ...
                    <input type="hidden" class="form-control col-sm-4" id="saldo_normal" readonly>
                    <th>
                      <select class="form-control select2" name="kode_jenjang2" id="kode_jenjang2" required="">
                        <option value="">Pilih Jenjang</option>
                        <?php
                                $query = "SELECT * FROM master_jenjang";
                                $result = $konektor->query($query);
              ...
              <tr>
                <th>
                <select class="form-control select2" name="kode_akun1" id="kode_akun1" >
                  <option value="">Pilih Akun</option>
                <?php
                $query = "SELECT * FROM akun WHERE status = 1";
                $result = $konektor->query($query);
...
if (isset($_POST['simpan'])) {
    ...
    if ($jenis_transaksi === 'Penerimaan' && !empty(saldo1)) {
    ...
    } if ($jenis_transaksi === 'Pengeluaran' && !empty(saldo1)) {
      $insertquery2 = "INSERT INTO transaksi_kas (no_transaksi, jenis_transaksi, kode_akun, nama_akun, akun_kas, nama, tanggal, sumber_dana, kode_tahun, tahun_ajaran, kode_jenjang, nama_jenjang, keterangan, kredit, debit, status) 
    ...
    } if ($jenis_transaksi === 'Penerimaan' && !empty(saldo2)) {
      $insertquery3 = "INSERT INTO transaksi_kas (no_transaksi, jenis_transaksi, kode_akun, nama_akun, akun_kas, nama, tanggal, sumber_dana, kode_tahun, tahun_ajaran, kode_jenjang, nama_jenjang, keterangan, debit, kredit, status) 
      VALUES('$kode_transaksi', '$jenis_transaksi','$kode_akun2','$nama_akun2','$akun_kas','$nama','$tanggal','$sumber_dana','$kode_tahun','$tahun_ajaran','$kode_jenjang2','$nama_jenjang2','$keterangan2','$total','$saldo2','$status')";  
    
    }if ($jenis_transaksi === 'Pengeluaran' && !empty(saldo2)) {
      $insertquery4 = "INSERT INTO transaksi_kas (no_transaksi, jenis_transaksi, kode_akun, nama_akun, akun_kas, nama, tanggal, sumber_dana, kode_tahun, tahun_ajaran, kode_jenjang, nama_jenjang, keterangan, kredit, debit, status) 
      VALUES('$kode_transaksi', '$jenis_transaksi','$kode_akun2','$nama_akun2','$akun_kas','$nama','$tanggal','$sumber_dana','$kode_tahun','$tahun_ajaran','$kode_jenjang2','$nama_jenjang2','$keterangan2','$totalField','$saldo2','$status')";
    }
