...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($data = $result1->fetch_assoc()) {
        $budget_data[$data['kode_akun']] = [
            ...
            'total_digunakan' => 0 // Default total digunakan = 0
        ];
    }
    ...
    $query2 = "SELECT 
                ...
                akun.saldo_normal, 
                SUM(
                    (SELECT COALESCE(SUM(debit), 0) FROM transaksi_kas 
                     WHERE status = 1 AND budgeting.kode_akun = transaksi_kas.kode_akun 
                     AND transaksi_kas.tahun_ajaran = ? AND transaksi_kas.nama_jenjang = ?)
                ) +
                SUM(
                    (SELECT COALESCE(SUM(debit), 0) FROM transaksi_bank 
                     WHERE status = 1 AND budgeting.kode_akun = transaksi_bank.kode_akun 
                     AND transaksi_bank.tahun_ajaran = ? AND transaksi_bank.nama_jenjang = ?)
                ) +
                SUM(
                    (SELECT COALESCE(SUM(debit), 0) FROM transaksi_memorial 
                     WHERE status = 1 AND budgeting.kode_akun = transaksi_memorial.kode_akun 
                     AND transaksi_memorial.tahun_ajaran = ? AND transaksi_memorial.nama_jenjang = ?)
                ) AS total_debit,

                SUM(
                    (SELECT COALESCE(SUM(kredit), 0) FROM transaksi_kas 
                     WHERE status = 1 AND budgeting.kode_akun = transaksi_kas.kode_akun 
                     AND transaksi_kas.tahun_ajaran = ? AND transaksi_kas.nama_jenjang = ?)
                ) +
                SUM(
                    (SELECT COALESCE(SUM(kredit), 0) FROM transaksi_bank 
                     WHERE status = 1 AND budgeting.kode_akun = transaksi_bank.kode_akun 
                     AND transaksi_bank.tahun_ajaran = ? AND transaksi_bank.nama_jenjang = ?)
                ) +
                SUM(
                    (SELECT COALESCE(SUM(kredit), 0) FROM transaksi_memorial 
                     WHERE status = 1 AND budgeting.kode_akun = transaksi_memorial.kode_akun 
                     AND transaksi_memorial.tahun_ajaran = ? AND transaksi_memorial.nama_jenjang = ?)
                ) AS total_kredit
    ...
    foreach ($dataRows as $row) {
        ...
        echo "<td>{$total_nominal}</td>";
        echo "<td>{$total_realisasi}</td>";
        echo "<td>{$presentase_penggunaan}</td>";
        echo "</tr>";
    }
